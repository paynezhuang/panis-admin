...
const model = defineModel<Api.SystemManage.PositionSearchParams>('model', { required: true });
      ...
      <NGrid responsive="screen" item-responsive :x-gap="8" :y-gap="8" cols="1 s:1 m:5 l:5 xl:5 2xl:5">
        <NGridItem span="4">
          <NGrid responsive="screen" item-responsive :x-gap="8">
            <NFormItemGi span="24 s:8 m:6" :label="$t('page.manage.position.name')" path="roleName">
              <NInput v-model:value="model.name" size="small" :placeholder="$t('page.manage.position.form.name')" />
            </NFormItemGi>

            <NFormItemGi span="24 s:8 m:6" :label="$t('page.manage.position.status')" path="status">
              <NSelect
